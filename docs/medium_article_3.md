...
In Part 1, we ingested a Lord of the Rings API into Salesforce Data Cloud using the Ingestion API — no connector, no middleware. 933 characters and 2,383 quotes landed in Data Cloud.
...
But data in Data Cloud alone isn't the destination.
...
Data Cloud doesn't automatically know how to link external data to your CRM records. You have to build the bridge.
...
When Identity Resolution runs, Data Cloud creates a unified profile that links Treebeard's external profile data to his Salesforce Person Account.
...
For our LOTR example, we created a Salesforce Flow that triggers when a new `LotrCharacter` record appears in Data Cloud:

1. **Trigger:** Data Cloud Data Change Event on `lotr_LotrCharacter__dlm`
2. **Action:** Create Person Account
3. **Mapping:** Map Data Cloud fields to Account fields:
   - `name__c` → `LastName`
   - `race__c` → `Race__c`
   - `height__c` → `Height__c`
...
*Salesforce Flow automatically creates Person Accounts when Data Cloud profiles are created*

When Treebeard's character data lands in Data Cloud, the Flow creates his Person Account in CRM within minutes.

This is the power of Data Cloud automation: external data triggers CRM actions without custom middleware or scheduled jobs.

---

## The UI: Related Lists

Data Cloud Related Lists bring Engagement data directly into Salesforce CRM record pages.
...
All 45 quotes from Treebeard appear directly on his Account page, powered by Data Cloud Engagement data.
...
2. **Activity data** — Treebeard's 45 quotes (Data Cloud Engagement)
...
2. **Ingestion API** → Python script sends data to Data Cloud
3. **Data Cloud** → Profile (LotrCharacter) and Engagement (LotrQuote) records are created
4. **Identity Resolution** → Data Cloud links `LotrCharacter.characterId` to `Account.characterId__c`
5. **Salesforce Flow** → Flow creates Person Account when Data Cloud profile is created
6. **Related List** → Quotes appear on Account page via Data Cloud Related List
...
*The complete journey: External API → Data Cloud → CRM → Agentforce*
...
3. **Deploy Salesforce Flow** — Automate Account creation from Data Cloud
...
The Ingestion API isn't just about getting data into Data Cloud. It's about making external data first-class in Salesforce.
...
- **Part 1:** Ingesting external APIs into Data Cloud using the Ingestion API
- **Part 2:** Modeling Profiles and Engagements correctly
- **Part 3:** Bridging Data Cloud to CRM with Identity Resolution, Flows, Related Lists, and Agentforce
...
- [Identity Resolution in Data Cloud](https://help.salesforce.com/s/articleView?id=sf.c360a_identity_resolution.htm)
- [Data Cloud Related Lists](https://help.salesforce.com/s/articleView?id=sf.c360a_related_lists.htm)
